module Scruffy
  module Box
    class Cloud < Base
      ...
      def self.find pinky_id, *c, &b
        cb = EM::Callback(*c, &b)
        EM.defer(proc {
            compute_cloud.servers.get(pinky_id)
          }, proc { |vm|
...
apt-get -y install lzop
      ...
      def initialize vm
        ...
        @pinky_id, @instance_type, @host = vm.id, vm.flavor_id, vm.public_ip_address
      ...
      def start
        ...
        EM.defer(proc {
            begin
              ...
              puts "start box:#{pinky_id} failed: #{e}"
              df.fail e
              false
            end
          }, proc { |succeeded|
      ...
      def query_aws_state timeout = 20, *c,&b
