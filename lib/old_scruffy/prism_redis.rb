...
module Prism
  class << self
    ...
    def hiredis_connect
      @redis_factory ? @redis_factory.call : EM::Hiredis.connect(ENV['REDIS_URI'] || REDIS_URI)
    end
  ...
  class PrismRedis
    ...
    def store_running_box box
      zadd "boxes:stopped", "inf", box.pinky_id
      hset_hash "workers:running", box.pinky_id, box.to_hash
    end

    def unstore_running_box pinky_id, host
      zadd "boxes:stopped", Time.now.to_i, pinky_id
      hdel "workers:running", pinky_id
      publish "workers:requests:stop:#{pinky_id}", host
    end

    def store_running_world world_id, pinky_id, host, port, slots
      world_hash = {
        pinky_id: pinky_id,
        host: host,
        port: port,
    ...
    def unstore_running_world pinky_id, world_id
    ...
    def get_json_df key, *a, &b
      df = EM::DefaultDeferrable.new
      op = get key
      op.callback {|data| df.succeed data ? JSON.parse(data) : nil }
