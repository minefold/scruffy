...
module Scruffy
  ...
  class BoxType
    ...
    def initialize instance_type
      ...
      @image_id         = instance[:instanceType]
  ...
  class WorldAllocator
    ...
    def new_instance_type
      ...
      big_boys = running_pinky_capacities.count {|b| b['instance_type'] == 'cc2.8xlarge' }
    ...
    def start_options_for_new_world world, player_slots_required = nil
      ...
      # TODO: this probably belongs in the scruffy (rebalancer)
    ...
    def find_box_for_new_world world, player_slots_required
      ...
      if box = running_pinky_capacities.find{|box| Array(worlds_accepted(box)).include? world['_id'] }
        debug "using dedicated box:#{box['pinky_id']}"
        box

      else
        candidates = pinkies_with_capacity.select do |box|
          debug "candidate:#{box["pinky_id"]}  world_slots:#{box[:world_slots]}  player_slots:#{box[:player_slots]}"
          box_type = BoxType.new(box['instance_type'])
    ...
    def shutdown_idle_boxes
      ...
      idles_close_to_end_of_hour = idle_boxes.select{|pinky_id, box| close_to_end_of_hour box }

      debug "available world slots:#{total_world_slots} idle boxes:#{idle_boxes.size} killable:#{idles_close_to_end_of_hour.size}"

      idles_close_to_end_of_hour.each do |pinky_id, box|
        box_type = BoxType.new(box['instance_type'])
        excess_capacity -= box_type.world_cap
        if excess_capacity >= 0
          box = idles_close_to_end_of_hour[pinky_id]

          message = box[:description]
          info "#{message} terminating idle"
          Prism.redis.lpush "workers:requests:stop", pinky_id
        end
      end
    end

    def shutdown_boxes_not_in_use
      unusable_boxes.select {|box| instanceType.size == 0 && box[:players].size == 0 && close_to_end_of_hour(box) }.each do |box|
        message = box[:description]
        info "#{message} terminating unuseable"
        Prism.redis.lpush "workers:requests:stop", box['pinky_id']
      end
    end
    ...
    def print_box_status
      pinkies_creating.each do |request_id, pinky|
        box_type = BoxType.new(box['instance_type'])
        debug "box:#{request_id} slots:#{box_type.world_cap} players:#{box_type.player_cap} creating..."
      end

      running_pinky_capacities.each do |pinky|
        box = find_box(pinky.pinky_id)
        box_type = BoxType.new(box.instance_type)
        # widget = universe.widgets[box['pinky_id']]
        #
        # if widget
        #   if instanceType.any?
        #     worlds_info = instanceType.each_with_object({}) do |(id, w), h|
        #       h[id] = {
        ...
        #     debug "box:#{box['pinky_id']} worlds:#{descriptions.join(' ')} total:[#{player_total} #{cpu_total}% #{mem_total.to_human_size}]"
        #   end
        #
        #   if disk = widget['disk']
        #     debug "box:#{box['pinky_id']} disk: #{disk.values.map{|d| ((d['used'] / (d['total'] || 1).to_f) * 100).to_i.to_s + "%" }.join(' ')}"
        #   end
        # end
    ...
    def pinkies_with_capacity
      @pinkies_with_capacity ||= running_pinky_capacities.select {|box| has_capacity?(box) }.reject{|box| worlds_accepted(box) }
    end

    def unusable_boxes
      @unusable_boxes ||= running_pinky_capacities.select {|box| worlds_accepted(box) }
    end

    def idle_boxes
      # boxes that have no players, no worlds and are accepting new worlds
      @idle_boxes ||= begin
        non_busy_boxes.select do |pinky_id, box|
          p boxes[pinky_id]
    ...
    def find_box pinky_id
      @boxes.find{|b| b.pinky_id == pinky_id }
    end

    def running_pinky_capacities
      @running_pinky_capacities ||= begin
        universe.pinkies.map do |pinky_id, pinky|
          box = find_box(pinky_id)
          p pinky_id, @boxes, box

          box_type = BoxType.new box.instance_type
          pinky[:player_slots] = box_type.player_cap - pinky[:players].size
          pinky[:world_slots] = box_type.world_cap - pinky[:servers].inject(0) {|sum, (id, w)| sum + (w['slots'] || 1)}
          pinky[:description] = {
            box: pinky_id,
            worlds: pinky[:servers].size,
            slots: "#{box_type.world_cap - pinky[:world_slots]}/#{box_type.world_cap}",
            uptime: friendly_time(box.uptime)
          }.map{|k,v| "#{k}:#{v}" }.join(' ')
    ...
    def running_world_slots
      pinkies_with_capacity.inject(0) {|acc, box| acc + box[:world_slots] }
    end

    def upcoming_world_slots
      pinkies_creating.inject(0) {|acc, (_, box)| acc + BoxType.new(box['instance_type']).world_cap }
    end

    def pinkies_creating
      universe.scruffy_pinkies.select{|pinky_id, pinky| pinky[:state] == 'creating' }
    end
